...
var scout = {
    run: function(creep) {
		...
		let closestMyRoomDistanceToCheckSources = 6;
		let closestMyRoomDistanceToCheckPlayers = 6;
		let closestMyRoomDistanceToCheckPower = 8;
        ...
        if(creep.memory.currentRoom != creep.room.name) {
			...
			// We find the closest of room "globally", from bird's eye - "game.map.getRoomLinearDistance" is not expensive at all
			let myRooms = _.filter(Game.rooms, (currentRoom) => currentRoom.controller != undefined && currentRoom.controller.my);
			let closestMyRoomDistance = 1000;
			for(let currentRoomIndex = 0; currentRoomIndex < myRooms.length; currentRoomIndex++) {
				let distanceOfAssessedRoom = Game.map.getRoomLinearDistance(creep.room.name, myRooms[currentRoomIndex].name);
				if(distanceOfAssessedRoom < closestMyRoomDistance) {
					closestMyRoomDistance = distanceOfAssessedRoom;
				}
			}
			// Now we know that the closest of my rooms is at closestMyRoomDistance.
			
			// If my closest room is farther than the parameter setup rooms away, no way we'll harvest it {
			if(closestMyRoomDistance < closestMyRoomDistanceToCheckSources) {
			...
			if(closestMyRoomDistance < closestMyRoomDistanceToCheckPlayers) {
			...
			if(closestMyRoomDistance < closestMyRoomDistanceToCheckPower) {
